...
public class VerificationCodeServiceImpl extends ServiceImpl<VerificationCodeMapper, VerificationCode> implements IVerificationCodeService {
    ...
    public void sendRegisterCode(String username) {
        ...
        if (verificationCode != null){
            ...
            verificationCode.setCodeNumber(code);
            verificationCode.setIsActive(1);
            checkAndSendCode(verificationCode);
        }
        //调用通知模块，将code扔入死信队列
        ResponseBean responseBean = notificationFeignService.sendRegisterCode(verificationCode);
     ...
     */
    public void sendCode(VerificationCode verificationCode){
        String username = verificationCode.getCodeUsername();
        //判断是邮箱还是手机
        if (username.contains("@")){
            //发送邮箱验证码
            try{
                mailService.sendHtmlMail(verificationCode.getCodeUsername(),"PlanPlus注册",formatResgisterCodeContent(verificationCode.getCodeNumber()));
                //更新记录
     ...
     * @param verificationCode
     */
    public void checkAndSendCode(VerificationCode verificationCode){
        String username = verificationCode.getCodeUsername();
        //判断是邮箱还是手机
        if (username.contains("@")){
            //判断邮箱格式是否正确
     ...
     * @return
     */
    private String formatResgisterCodeContent(String code) {
